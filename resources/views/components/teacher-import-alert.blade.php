            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200 shadow-sm">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center space-x-3">
                        @if($status === 'warning' || !empty($stats['error_list']) || ($stats['skipped'] ?? 0) > 0)
                            <div class="bg-amber-100 p-2 rounded-lg">
                                <svg class="w-6 h-6 text-amber-600" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                </svg>
                            </div>
                        @elseif(($stats['success'] ?? 0) > 0)
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-600 mt-1">Total <span class="font-semibold">{{ $stats['processed_rows'] ?? 0 }}</span> data diproses</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="border border-green-200 rounded-xl overflow-hidden bg-white shadow-sm">
                    <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-4 py-3 border-b border-green-200">
                        <h4 class="font-semibold text-green-800 flex items-center">
                            ...
                            <span class="ml-2 bg-green-200 text-green-800 px-2.5 py-0.5 rounded-full text-xs font-medium">{{ count($stats['success_data']) }} guru</span>
                        </h4>
                    </div>
                    <div class="max-h-80 overflow-y-auto">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach(array_slice($stats['success_data'], 0, 50) as $index => $item)
                                    <tr class="hover:bg-green-50">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-slate-600 font-mono">{{ $item['nip'] ?? '-' }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-blue-600">{{ $item['kelas'] ?? '-' }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-purple-600">{{ $item['mapel'] ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        @if(count($stats['success_data']) > 50)
                            <div class="bg-gradient-to-r from-gray-50 to-green-50 px-4 py-3 border-t border-gray-200">
                                <p class="text-sm text-gray-600 text-center">dan {{ count($stats['success_data']) - 50 }} data lainnya</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="border border-amber-200 rounded-xl overflow-hidden bg-white shadow-sm">
                    <div class="bg-gradient-to-r from-amber-50 to-orange-50 px-4 py-3 border-b border-amber-200">
                        <h4 class="font-semibold text-amber-800 flex items-center">
                            ...
                            Data Dilewati
                            <span class="ml-2 bg-amber-200 text-amber-800 px-2.5 py-0.5 rounded-full text-xs font-medium">{{ count($stats['skipped_data']) }} data</span>
                        </h4>
                    </div>
                    <div class="max-h-80 overflow-y-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @if(count($stats['skipped_data']) > 30)
                            <div class="bg-gradient-to-r from-gray-50 to-amber-50 px-4 py-3 border-t border-gray-200">
                                <p class="text-sm text-gray-600 text-center">dan {{ count($stats['skipped_data']) - 30 }} data lainnya</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="border border-red-200 rounded-xl overflow-hidden bg-white shadow-sm">
                    <div class="bg-gradient-to-r from-red-50 to-pink-50 px-4 py-3 border-b border-red-200">
                        <h4 class="font-semibold text-red-800 flex items-center">
                            ...
                            Kesalahan
                            <span class="ml-2 bg-red-200 text-red-800 px-2.5 py-0.5 rounded-full text-xs font-medium">{{ count($stats['error_list']) }} error</span>
                        </h4>
                    </div>
                    <div class="max-h-72 overflow-y-auto">
                        <div class="divide-y divide-red-100">
                            @foreach(array_slice($stats['error_list'], 0, 15) as $index => $error)
                                <div class="px-4 py-3 hover:bg-red-50 flex items-start">
                                    <span class="flex-shrink-0 w-6 h-6 rounded-full bg-red-100 text-red-600 text-xs font-bold flex items-center justify-center mr-3">{{ $index + 1 }}</span>
                        ...
                        @if(count($stats['error_list']) > 15)
                            <div class="bg-gradient-to-r from-gray-50 to-red-50 px-4 py-3 border-t border-red-200">
                                <p class="text-sm text-gray-600 text-center">dan {{ count($stats['error_list']) - 15 }} error lainnya</p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            @foreach($stats['created_classes'] as $className => $count)
                                <div class="flex justify-between items-center bg-gradient-to-r from-blue-50 to-indigo-50 p-3 rounded-lg border border-blue-200">
                                    <span class="font-semibold text-slate-700">{{ $className }}</span>
                                    <span class="bg-blue-500 text-white px-3 py-1 rounded-full text-xs font-bold">{{ $count }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
