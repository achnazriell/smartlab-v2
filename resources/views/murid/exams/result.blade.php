            ...
            <div class="bg-white rounded-2xl p-6 mb-8 border-2 border-blue-100 shadow-sm">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="flex-1 space-y-5">
                        ...
                        <div class=" gap-4 text-slate-600">
                    ...
                    <div class="bg-blue-600 text-white rounded-xl p-6 text-center shadow-sm border-2 border-blue-700">
                        ...
                        <div class="text-sm opacity-90">
                            ...
                            @endphp
                            <span
                                class="inline-block px-3 py-1 rounded-full {{ $color }} text-white text-xs font-semibold">
                                {{ $grade }}
                            </span>
                        </div>
                    </div>
            ...
            @if ($attempt->is_cheating_detected || $attempt->violation_count > 0)
                <div class="mb-8 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-red-800">Ujian Dihentikan Karena Pelanggaran</h3>
                            <p class="text-red-700">
                                Ujian telah dihentikan otomatis karena {{ $attempt->violation_count }} pelanggaran.
                                @if ($attempt->is_cheating_detected)
                                    <span class="font-bold">Terdeteksi kecurangan!</span>
                                @endif
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 border-2 border-blue-100 shadow-sm mb-8">
                ...
                <div class="space-y-6">
                    @foreach ($attempt->answers->sortBy('question.order') as $index => $answer)
                        ...
                        <div
                            class="border border-slate-200 rounded-xl p-6 hover:border-slate-300 transition-colors {{ $isCorrect ? 'bg-green-50' : 'bg-red-50' }}">
                                    ...
                                    <div class="space-y-2">
                                        @foreach ($question->choices->sortBy('order') as $choice)
                                            <div
                                                ...
                                                <div class="flex-1">
                                                    <p
                                                        class="{{ $answer->choice_id == $choice->id ? 'font-medium' : '' }}">
                                                        {{ $choice->text }}
                                                    </p>
                                                </div>
                                ...
                                <div
                                    ...
                                    <p class="{{ $isCorrect ? 'text-green-800' : 'text-red-800' }} font-medium">
                                        @if ($isCorrect)
                                            ...
                                            <span class="font-bold">{{ $answer->choice->label ?? '' }}.
                                                {{ $answer->choice->text ?? '' }}</span>
                                        @else
                                            ...
                                            @if ($correctChoice)
                                                <br>
                                                ✓ Jawaban yang benar:
                                                <span class="font-bold">{{ $correctChoice->label }}.
                                                    {{ $correctChoice->text }}</span>
                                            @endif
                                        @endif
                                    </p>
                                </div>
                                    ...
                                    @if ($exam->show_correct_answer)
                                        <div>
                                            ...
                                            <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                                                ...
                                                @else
                                                    <p class="text-green-600 font-medium">Tidak ada jawaban benar yang
                                                        ditetapkan</p>
                                                @endif
                                            </div>
                                        </div>
