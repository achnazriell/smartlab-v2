        ...
        <aside class="fixed top-0 left-0 h-full bg-white border-r shadow-xl z-50 transition-all duration-300"
            ...
            <nav class="mt-4 px-3 space-y-1 overflow-y-auto h-[calc(100vh-80px)] pb-10">
                ...
                <a href="{{ route('guru.exams.index') }}"
                    class="flex items-center py-3 px-4 rounded-xl transition-all duration-200 group relative {{ request()->routeIs('guru.exams*') ? 'sidebar-item-active' : 'text-slate-600 sidebar-item-hover' }}"
                    :class="(sidebarOpen || mobileSidebarOpen) ? '' : 'justify-center'"
                    ...
                    <span x-show="sidebarOpen || mobileSidebarOpen"
                        class="font-medium text-md whitespace-nowrap">Ujian</span>
                </a>
                <a href="{{ route('guru.quiz.index') }}"
                    class="flex items-center py-3 px-4 rounded-xl transition-all duration-200 group relative {{ request()->routeIs('guru.quiz*') ? 'sidebar-item-active' : 'text-slate-600 sidebar-item-hover' }}"
                    :class="(sidebarOpen || mobileSidebarOpen) ? '' : 'justify-center'"
                    :title="(sidebarOpen || mobileSidebarOpen) ? '' : 'Quiz'">
                    <svg class="w-6 h-6 flex-shrink-0" :class="(sidebarOpen || mobileSidebarOpen) ? 'mr-3' : ''"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
                    ...
                    <span x-show="sidebarOpen || mobileSidebarOpen"
                        class="font-medium text-md whitespace-nowrap">Quiz</span>
                </a>
            </nav>
        </aside>
