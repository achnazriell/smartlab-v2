    ...
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-400 to-blue-200 rounded-2xl p-8 text-white shadow-lg">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-3xl md:text-4xl font-bold text-white">Kelola Pengumpulan</h1>
                </div>
            </div>

            <!-- Search inline dengan styling modern -->
            <form action="{{ route('collections.byTask', $task->id) }}" method="GET" class="flex-1 mt-6">
                <input type="text" name="search" placeholder="Cari pengumpulan..." value="{{ request('search') }}"
                    class="w-full px-4 py-2.5 rounded-xl text-white placeholder-white outline-none text-sm md:text-base ring-2 ring-white">
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-blue-50">
                <h3 class="text-lg md:text-xl font-bold text-gray-900">Daftar Pengumpulan</h3>
                <p class="text-xs md:text-sm text-gray-600 mt-1">Total: <span
                        class="font-semibold text-blue-600">{{ $collections->total() ?? 0 }}</span> pengumpulan</p>
            </div>
                        ...
                        @forelse ($collections as $index => $collection)
                            ...
                            <tr class="hover:bg-blue-50/50 transition-colors">
                                ...
                                <td class="px-4 md:px-6 py-4">
                                    <span
                                        class="text-xs md:text-sm font-semibold text-gray-900">{{ $collection->Task->title_task }}</span>
                                </td>
                                <td class="px-4 md:px-6 py-4 text-sm text-gray-900">{{ $collection->user->name }}</td>
                                ...
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center">
                                        <button onclick="openModal('showAssessmentModal_{{ $collection->id }}')"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 md:px-6 py-12 text-center">
                                    ...
                                    <p class="text-xs md:text-sm text-gray-500">Siswa akan mulai mengumpulkan tugas di sini
                                    </p>
                                </td>
                            </tr>
                        @endforelse
        ...
        <!-- Detail modals dengan styling modern -->
            ...
            <div id="showAssessmentModal_{{ $collection->id }}"
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    <div
                        ...
                        <h5 class="text-xl md:text-2xl font-bold text-white">Detail Pengumpulan</h5>
                    ...
                    <div class="overflow-y-auto flex-1 px-6 md:px-8 py-6 space-y-5">
                        <div>
                            <h6 class="text-lg font-semibold text-gray-700 mb-2">Nama Tugas:</h6>
                            <p class="text-gray-600 text-sm md:text-base">{{ $collection->Task->title_task }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            <h6 class="text-lg font-semibold text-gray-700 mb-2">Kelas:</h6>
                        ...
                        <div>
                            <h6 class="text-lg font-semibold text-gray-700 mb-3">Bukti Pengumpulan:</h6>
                            ...
                            @if (in_array($file, ['jpg', 'png', 'jpeg']))
                                <img src="{{ asset('storage/' . $collection->file_collection) }}" alt="File Image"
                                    class="w-auto h-48 border-2 border-gray-200 rounded-lg">
                            @elseif($file === 'pdf')
