...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-blue-700 to-blue-500 rounded-2xl p-8 text-white shadow-lg">
        ...
        {{-- FILTER ROW --}}
        <form method="GET" action="{{ route('materis.index') }}" class="mt-6 flex flex-col sm:flex-row gap-3 flex-wrap">
            ...
            <select name="mapel"
                ...
                <option value="{{ $m->id }}" class="text-slate-800" @selected(request('mapel') == $m->id)>{{ $m->name }}</option>
                @endforeach
            </select>
            <select name="tipe"
                class="px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white text-sm outline-none focus:bg-white/20 transition">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        @php
            ...
            $pdfCount   = $allMateri->filter(fn($m) => strtolower(pathinfo($m->file_materi ?? '', PATHINFO_EXTENSION)) === 'pdf')->count();
            $videoCount = $allMateri->filter(fn($m) => in_array(strtolower(pathinfo($m->file_materi ?? '', PATHINFO_EXTENSION)), ['mp4','webm','avi','mov']))->count();
            $mapelCount = $allMateri->pluck('subject_id')->unique()->count();
            $statList = [
                ['label' => 'Total Materi',   'value' => $total,      'icon' => 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253', 'color' => 'bg-blue-50 text-blue-600'],
                ['label' => 'Berkas PDF',     'value' => $pdfCount,   'icon' => 'M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z', 'color' => 'bg-rose-50 text-rose-500'],
                ['label' => 'Video',          'value' => $videoCount, 'icon' => 'M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z', 'color' => 'bg-violet-50 text-violet-600'],
                ['label' => 'Mata Pelajaran', 'value' => $mapelCount, 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', 'color' => 'bg-amber-50 text-amber-500'],
            ];
        @endphp
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100">
            ...
            <p class="text-xs text-slate-500 mt-0.5">
                Menampilkan <span class="font-semibold text-blue-600">{{ method_exists($materis, 'total') ? $materis->total() : $materis->count() }}</span> materi
            </p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Judul Materi</th>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider hidden md:table-cell">Tipe</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider hidden lg:table-cell">Diunggah</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php
                        ...
                        $tipeConfig = match(true) {
                            $ext === 'pdf'                          => ['label' => 'PDF',    'color' => 'bg-rose-100 text-rose-700',   'icon' => 'M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z'],
                            in_array($ext, ['mp4','webm','mov'])    => ['label' => 'Video',  'color' => 'bg-violet-100 text-violet-700','icon' => 'M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z'],
                            in_array($ext, ['doc','docx'])          => ['label' => 'Word',   'color' => 'bg-blue-100 text-blue-700',   'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                            in_array($ext, ['ppt','pptx'])          => ['label' => 'PPT',    'color' => 'bg-orange-100 text-orange-700','icon' => 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z'],
                            !empty($materi->link_materi)            => ['label' => 'Link',   'color' => 'bg-cyan-100 text-cyan-700',   'icon' => 'M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14'],
                            default                                 => ['label' => 'File',   'color' => 'bg-slate-100 text-slate-600', 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                        };
                    @endphp
                    <tr class="hover:bg-blue-50/40 transition-colors">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-xl {{ $tipeConfig['color'] }} flex items-center justify-center flex-shrink-0">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $tipeConfig['icon'] }}"/>
                                    </svg>
                                </div>
                                <div class="min-w-0">
                                    <p class="font-semibold text-slate-800 truncate">{{ $materi->title_materi }}</p>
                                    @if ($materi->deskripsi ?? false)
                                    <p class="text-xs text-slate-400 truncate max-w-[220px]">{{ $materi->deskripsi }}</p>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4 hidden lg:table-cell">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-100 text-slate-700 mr-1 mb-1">
                                {{ $c->name_class }}
                            </span>
                            @endforeach
                        </td>
                        <td class="px-5 py-4 hidden md:table-cell">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold {{ $tipeConfig['color'] }}">
                                {{ $tipeConfig['label'] }}
                            </span>
                        </td>
                        ...
                        <td class="px-5 py-4 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <button onclick="openModal('detailModal_{{ $materi->id }}')"
                                    class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Lihat Detail">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div id="detailModal_{{ $materi->id }}"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div class="bg-gradient-to-r from-blue-700 to-blue-500 px-7 py-5 flex justify-between items-center">
                ...
                <button onclick="closeModal('detailModal_{{ $materi->id }}')" class="text-white/70 hover:text-white transition">
            ...
            <div class="overflow-y-auto flex-1 px-7 py-6 space-y-5">
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-slate-50 rounded-xl p-4">
                        <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Mata Pelajaran</p>
                        <p class="text-slate-800 font-semibold text-sm">{{ $materi->subject->name ?? '—' }}</p>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4">
                        <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Kelas</p>
                        <div class="flex flex-wrap gap-1 mt-1">
                            @foreach ($materi->classes ?? [] as $c)
                            <span class="px-2.5 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-800">{{ $c->name_class }}</span>
                            @endforeach
                        </div>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4">
                        <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Diunggah</p>
                        <p class="text-slate-800 font-semibold text-sm">{{ $materi->created_at?->format('d M Y, H:i') ?? '—' }}</p>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4">
