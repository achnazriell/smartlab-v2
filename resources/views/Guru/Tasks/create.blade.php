...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-blue-50 border-l-4 border-blue-400 rounded-xl shadow-sm p-4 md:p-8">
            <div class="min-w-0">
                ...
                <p class="text-sm md:text-base text-blue-700 mt-1">Buat tugas pembelajaran untuk kelas Anda</p>
            </div>
        </div>
            ...
            <form action="{{ route('tasks.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="title_task" name="title_task"
                        ...
                        value="{{ old('title_task') }}" placeholder="Contoh: PR Latihan Soal" required>
                    @error('title_task')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <!-- Mapel -->
                    <div>
                        <label for="subject_id" class="block text-sm font-semibold text-gray-900 mb-2.5">
                            Mapel <span class="text-red-600">*</span>
                        </label>
                    ...
                    <!-- Tanggal Pengumpulan -->
                    <div>
                        <label for="date_collection" class="block text-sm font-semibold text-gray-900 mb-2.5">
                            Tanggal & Waktu Pengumpulan <span class="text-red-600">*</span>
                        </label>
                ...
                <div>
                    ...
                    <textarea id="description_task" name="description_task" rows="4"
                        ...
                        placeholder="Jelaskan instruksi tugas...">{{ old('description_task') }}</textarea>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-600 mb-3">Format: PDF, JPG, PNG. Maksimal 10MB</p>
                    <div class="relative">
    ...
    <script>
        ...
        function renderClassOptions() {
            ...
            if (options.length === 0) {
                classOptionsDiv.innerHTML = '<div class="p-4 text-center text-sm text-gray-500">Pilih mapel terlebih dahulu</div>';
                return;
            }
        ...
        function updateButtonChips() {
            ...
            const chipsHtml = selected.map(option => {
                return `
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
                        ${option.textContent}
                        <button type="button" class="remove-chip" data-value="${option.value}" onclick="removeChip(event, '${option.value}')">
                            <svg class="w-3.5 h-3.5 hover:text-blue-900" fill="currentColor" viewBox="0 0 20 20">
        ...
        function loadBySubject() {
            ...
            fetch(`/guru/subjects/${subjectId}/classes`)
                ...
                .then(data => {
                    if (!data.length) {
                        classOptionsDiv.innerHTML = '<div class="p-4 text-center text-sm text-gray-500">Kelas tidak tersedia</div>';
                        return;
                    }
                    data.forEach(item => {
            ...
            fetch(`/guru/subjects/${subjectId}/materi`)
                ...
                .then(data => {
                    data.forEach(item => {
                        materiSelect.innerHTML += `<option value="${item.id}">${item.title_materi ?? item.name_materi}</option>`;
                    });
                });
        }

        function loadByMateri() {
            ...
            fetch(`/guru/materi/${materiId}/kelas`)
                .then(res => res.json())
                .then(data => {
                    if (!data.length) {
                        classOptionsDiv.innerHTML = '<div class="p-4 text-center text-sm text-gray-500">Kelas tidak tersedia</div>';
