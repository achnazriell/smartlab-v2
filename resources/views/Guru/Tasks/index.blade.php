    ...
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-700 to-blue-500 rounded-2xl p-8 text-white shadow-lg">
            ...
            <form method="GET" action="{{ route('tasks.index') }}" class="mt-6 flex flex-col sm:flex-row gap-3">
                ...
                <select name="sort"
                    ...
                    <option value="deadline_asc" class="text-slate-800" @selected(request('sort') === 'deadline_asc')>Deadline Terdekat
                    </option>
                    <option value="deadline_desc" class="text-slate-800" @selected(request('sort') === 'deadline_desc')>Deadline Terjauh
                    </option>
                ...
                @if (request()->hasAny(['search', 'kelas', 'mapel', 'status', 'sort']))
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @php
                ...
                $aktifCount = $allTasks
                    ->filter(fn($t) => $t->deadline && \Carbon\Carbon::parse($t->deadline)->isFuture())
                    ->count();
                $berakhirCount = $allTasks
                    ->filter(fn($t) => $t->deadline && \Carbon\Carbon::parse($t->deadline)->isPast())
                    ->count();
                $noDeadline = $allTasks->filter(fn($t) => !$t->deadline)->count();
                $taskStats = [
                    [
                        'label' => 'Total Tugas',
                        'value' => $totalTasks,
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                        Menampilkan <span
                            class="font-semibold text-blue-600">{{ $tasks->total() ?? $tasks->count() }}</span> tugas
                    </p>
                </div>
            </div>
                        ...
                        @forelse ($tasks as $index => $task)
                            ...
                            <tr class="hover:bg-blue-50/40 transition-colors">
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    @if ($task->description)
                                        <p class="text-xs text-slate-400 mt-0.5 truncate max-w-[200px]">
                                            {{ $task->description }}</p>
                                    @endif
                                </td>
                                ...
                                <td class="px-5 py-4 hidden lg:table-cell">
                                    @foreach ($task->classes ?? [] as $c)
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-100 text-slate-700 mr-1">
                                            {{ $c->name_class }}
                                        </span>
                                    @endforeach
                                </td>
                                ...
                                <td class="px-5 py-4 hidden md:table-cell">
                                    @if ($isExpired)
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-500">Berakhir</span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-100 text-emerald-700">Aktif</span>
                                    @endif
                                </td>
