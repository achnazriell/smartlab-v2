        ...
        <div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-blue-50">
                ...
                <p class="text-xs md:text-sm text-gray-600 mt-1">Total: <span
                        class="font-semibold text-blue-600">{{ $tasks->total() }}</span> tugas</p>
            </div>
                        ...
                        @forelse ($tasks as $index => $task)
                            ...
                            <tr class="hover:bg-blue-50/50 transition-colors">
                                ...
                                <td class="px-4 md:px-6 py-4">
                                    <span
                                        class="text-xs md:text-sm font-semibold text-gray-900">{{ $task->title_task }}</span>
                                </td>
                                <td
                                    class="px-4 md:px-6 py-4 text-xs md:text-sm text-gray-700 font-medium hidden lg:table-cell">
                                    {{ $task->Materi->subject->name_subject ?? '-' }}
                                </td>
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                    <!-- Update badge color dari teal ke blue -->
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                        {{ Str::limit($task->Materi->title_materi, 15) }}
                                    </span>
                                </td>
                                <td class="px-4 md:px-6 py-4 text-xs md:text-sm text-gray-600 hidden lg:table-cell">
                                    {{ $task->classes->pluck('name_class')->join(', ') }}
                                </td>
                                <td
                                    class="px-4 md:px-6 py-4 whitespace-nowrap text-xs md:text-sm text-gray-600 hidden md:table-cell">
                                    <span
                                        class="text-orange-600 font-medium">{{ \Carbon\Carbon::parse($task->date_collection)->translatedFormat('d M Y') }}</span>
                                </td>
                                    ...
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <a href="{{ route('collections.byTask', $task->id) }}"
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 md:w-5 h-4 md:h-5"
                                                viewBox="0 -960 960 960" fill="#4f46e5" stroke-width="2.6">
                                                <path
                                        ...
                                        <a href="{{ route('assesments', $task->id) }}"
                                            ...
                                            title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 md:w-5 h-4 md:h-5"
                                                viewBox="0 -960 960 960" fill="#22c55e" stroke-width="2.6">
                                                <path
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 md:px-6 py-12 text-center">
                                    ...
                                    <p class="text-xs md:text-sm text-gray-500">Mulai dengan membuat tugas pembelajaran
                                        pertama</p>
                                </td>
                            </tr>
                        @endforelse
                ...
                <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm pt-6 pb-6 px-6">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button type="button" class="text-gray-500 hover:text-gray-700"
                            onclick="closeModal('deleteModal_{{ $task->id }}')">
                    ...
                    <p class="text-sm md:text-base text-gray-600 mb-6">Apakah Anda yakin ingin menghapus tugas <strong
                            class="text-blue-600">{{ $task->title_task }}</strong>? Tindakan ini tidak dapat dibatalkan.
                    </p>
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    <div class="overflow-y-auto flex-1 px-6 md:px-8 py-6 space-y-5">
                        ...
                        <div class="border-t border-gray-200 pt-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    <h6 class="text-sm font-bold text-gray-700 uppercase tracking-wide">Batas Pengumpulan
                                    </h6>
                                ...
                                <div>
                                    <h6 class="text-sm font-bold text-gray-700 uppercase tracking-wide">Tanggal Pembuatan
                                    </h6>
                                    <p class="text-sm md:text-base text-gray-900 mt-2 font-medium">
                                        {{ \Carbon\Carbon::parse($task->created_at)->translatedFormat('d M Y') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-gray-200 pt-5">
                            ...
                            <p class="text-sm md:text-base text-gray-700 leading-relaxed">
                                {{ $task->description_task ?? 'Tidak ada deskripsi' }}</p>
                        </div>
