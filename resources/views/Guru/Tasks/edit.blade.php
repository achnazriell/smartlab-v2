    ...
    <div class="space-y-6">
        ...
        <div class="bg-blue-50 border-l-4 border-blue-400 rounded-xl shadow-sm p-4 md:p-8">
            <div class="min-w-0">
                ...
                <p class="text-sm md:text-base text-blue-700 mt-1">
                    Edit: <span class="font-semibold">{{ $task->title_task }}</span>
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-4 md:p-8">
            ...
            <form action="{{ route('tasks.update', $task->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <!-- Grid: Mapel, Materi, Kelas, Tanggal -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <select id="subject_id" name="subject_id"
                            ...
                            @foreach ($mapels as $mapel)
                                <option value="{{ $mapel->id }}"
                                    {{ old('subject_id', $task->subject_id) == $mapel->id ? 'selected' : '' }}>
                                    {{ $mapel->name_subject }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="class_id" name="class_id[]" multiple class="hidden">
                            @foreach ($classes as $class)
                                <option value="{{ $class->id }}"
                                    {{ in_array($class->id, old('class_id', $task->classes->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $class->name_class }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label for="date_collection" class="block text-sm font-semibold text-gray-900 mb-2.5">
                            Batas Pengumpulan <span class="text-red-600">*</span>
                        </label>
                ...
                <div>
                    ...
                    <textarea id="description_task" name="description_task" rows="4"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm resize-none">{{ old('description_task', $task->description_task) }}</textarea>
