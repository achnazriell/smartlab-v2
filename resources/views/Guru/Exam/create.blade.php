        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden" x-data="examForm()"
            ...
            <div class="p-6 border-b border-slate-200 transition-colors duration-300"
                ...
                <h2 class="text-xl font-bold font-poppins transition-colors"
                    ...
                    <span x-text="examType === 'QUIZ' ? 'Buat Quiz Interaktif' : 'Buat Ujian Baru'"></span>
                </h2>
            ...
            <form action="{{ route('guru.exams.store') }}" method="POST" class="p-6 space-y-8" id="examForm">
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="type" @change="updateType($event.target.value)"
                                ...
                                <option value="UH" {{ old('type', 'UH') == 'UH' ? 'selected' : '' }}>Ulangan Harian
                                </option>
                                <option value="UTS" {{ old('type') == 'UTS' ? 'selected' : '' }}>UTS</option>
                                <option value="UAS" {{ old('type') == 'UAS' ? 'selected' : '' }}>UAS</option>
                                <option value="QUIZ" {{ old('type') == 'QUIZ' ? 'selected' : '' }}>Quiz Interaktif
                                </option>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="security_level" id="security_level"
                                ...
                                class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none">
                                <option value="none" {{ old('security_level', 'none') == 'none' ? 'selected' : '' }}>
                                    Tidak Ada</option>
                                <option value="basic" {{ old('security_level') == 'basic' ? 'selected' : '' }}>Basic
                                    (Fullscreen + Block Tab Baru)</option>
                                <option value="strict" {{ old('security_level') == 'strict' ? 'selected' : '' }}>Strict
                                    (Basic + Anti Copy-Paste)</option>
                                <option value="custom" {{ old('security_level') == 'custom' ? 'selected' : '' }}>Custom
                                    (Atur Manual)</option>
                            </select>
                            <p class="text-xs text-slate-500">Mengatur tingkat pengawasan otomatis</p>
                        </div>

                        <!-- Custom Security Settings (hidden by default) -->
                        <div id="custom-security-settings" class="space-y-4 hidden">
                            <!-- Fullscreen Mode -->
                            <div class="flex items-center space-x-3 p-3 bg-blue-50 rounded-lg">
                                <input type="checkbox" name="fullscreen_mode" id="fullscreen_mode" value="1"
                                    {{ old('fullscreen_mode', 1) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center space-x-3 p-3 bg-blue-50 rounded-lg">
                                <input type="checkbox" name="block_new_tab" id="block_new_tab" value="1"
                                    {{ old('block_new_tab', 1) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded">
                            ...
                            <div class="flex items-center space-x-3 p-3 bg-blue-50 rounded-lg">
                                <input type="checkbox" name="prevent_copy_paste" id="prevent_copy_paste" value="1"
                                    {{ old('prevent_copy_paste', 1) ? 'checked' : '' }}
                                    class="w-4 h-4 text-blue-600 rounded">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center space-x-3 p-3 bg-red-50 rounded-lg">
                                <input type="checkbox" name="disable_violations" id="disable_violations"
                                    ...
                                    {{ old('disable_violations') ? 'checked' : '' }} class="w-4 h-4 text-red-600 rounded">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 hidden">
                            ...
                            <div class="flex items-center space-x-3 p-3 bg-purple-50 rounded-lg">
                                <input type="checkbox" name="require_camera" id="require_camera" value="1"
                                    {{ old('require_camera') ? 'checked' : '' }} class="w-4 h-4 text-purple-600 rounded">
                            ...
                            <div class="flex items-center space-x-3 p-3 bg-purple-50 rounded-lg">
                                <input type="checkbox" name="require_mic" id="require_mic" value="1"
                                    {{ old('require_mic') ? 'checked' : '' }} class="w-4 h-4 text-purple-600 rounded">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-1">
                            ...
                            <select name="show_result_after"
                                class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none">
                                <option value="never"
                                    {{ old('show_result_after', 'never') == 'never' ? 'selected' : '' }}>Tidak Pernah
                                </option>
                                <option value="immediately"
                                    {{ old('show_result_after') == 'immediately' ? 'selected' : '' }}>Selesai Mengerjakan
                                </option>
                                <option value="after_submit"
                                    {{ old('show_result_after') == 'after_submit' ? 'selected' : '' }}>Setelah Semua Siswa
                                    Submit</option>
                                <option value="after_exam"
                                    {{ old('show_result_after') == 'after_exam' ? 'selected' : '' }}>Setelah Waktu Ujian
                                    Berakhir</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="difficulty_level"
                                class="w-full px-4 py-2 rounded-lg border border-purple-300 focus:ring-2 focus:ring-purple-500 outline-none">
                                <option value="easy"
                                    {{ old('difficulty_level', 'medium') == 'easy' ? 'selected' : '' }}>Mudah</option>
                                <option value="medium"
                                    {{ old('difficulty_level', 'medium') == 'medium' ? 'selected' : '' }} selected>Sedang
                                </option>
                        ...
                        <div class="flex items-center space-x-3 p-3 bg-purple-50 rounded-lg">
                            <input type="checkbox" name="randomize_questions" value="1"
                                {{ old('randomize_questions') ? 'checked' : '' }} class="w-4 h-4 text-purple-600 rounded">
                            <label class="text-sm font-medium text-slate-700 cursor-pointer">
                                Acak Soal
                            </label>
                        </div>
        ...
        function examForm() {
            return {
                ...
                updateSecurityLevel(level) {
                    ...
                    // Jika disable_violations dicentang, matikan semua pengaturan keamanan
                    if (disableViolations && disableViolations.checked) {
                        ...
                        if (blockTab) {
                            blockTab.disabled = true;
                            blockTab.checked = false;
                        }
                        if (copyPaste) {
                            copyPaste.disabled = true;
