...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Header warna subtle seperti create, bukan gradient green terang -->
        <div class="bg-blue-50 border-l-4 border-blue-400 rounded-xl shadow-sm p-4 md:p-8">
            <div class="min-w-0">
                <h1 class="text-xl md:text-3xl font-bold text-blue-900">Ubah Materi</h1>
                <p class="text-sm md:text-base text-blue-700 mt-1">Edit: <span class="font-semibold">{{ $materi->title_materi }}</span></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-4 md:p-8">
            ...
            <form action="{{ route('materis.update', $materi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="title_materi" name="title_materi"
                        ...
                        value="{{ old('title_materi', $materi->title_materi) }}" required>
                    @error('title_materi')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <select name="subject_id" id="subject_id"
                            ...
                            @foreach ($mapels as $mapel)
                                <option value="{{ $mapel->id }}"
                                    {{ old('subject_id', $materi->subject_id) == $mapel->id ? 'selected' : '' }}>
                                    {{ $mapel->name_subject }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Kelas dengan custom dropdown dan chips untuk menampilkan pilihan -->
                    <div>
                        ...
                        <div class="relative" x-data="{ open: false, search: '' }" @click.away="open = false">
                            ...
                            <button type="button" @click="open = !open"
                                ...
                                <div class="flex flex-wrap gap-1 md:gap-1.5 flex-1 items-center">
                                    ...
                                    <span id="selected-count" class="text-gray-500 text-xs">
                                        {{ count(old('class_id', $materi->classes->pluck('id')->toArray())) > 0 ? '' : 'Pilih Kelas' }}
                                    </span>
                                </div>
                            ...
                            <div x-show="open" x-cloak
                                ...
                                <div id="class-options" class="overflow-y-auto flex-1 p-2 space-y-1">
                                    @foreach ($materi->classes as $selectedClass)
                                        @foreach ($classes as $class)
                                            @if ($class->id == $selectedClass->id)
                                                <label class="flex items-center gap-2.5 md:gap-3 px-2 md:px-3 py-2 md:py-2.5 hover:bg-blue-50 rounded-md cursor-pointer transition-colors class-option"
                                                    data-name="{{ $class->name_class }}">
                                                    <input type="checkbox" name="class_id[]" value="{{ $class->id }}" checked
                                                        class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 checkbox-class">
                                                    <span class="text-xs md:text-sm text-gray-700 flex-1">{{ $class->name_class }}</span>
                                                </label>
                                            @endif
                                        @endforeach
                                    @endforeach
                                    @foreach ($classes as $class)
                                        @unless ($materi->classes->pluck('id')->contains($class->id))
                                            <label class="flex items-center gap-2.5 md:gap-3 px-2 md:px-3 py-2 md:py-2.5 hover:bg-blue-50 rounded-md cursor-pointer transition-colors class-option"
                                                data-name="{{ $class->name_class }}">
                        ...
                        <select id="class_id" name="class_id[]" multiple class="hidden">
                            @foreach ($classes as $class)
                                <option value="{{ $class->id }}"
                                    {{ in_array($class->id, old('class_id', $materi->classes->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $class->name_class }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm resize-none">{{ old('description', $materi->description) }}</textarea>
                    @error('description')
                        <span class="text-red-600 text-xs mt-1.5 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if ($materi->file_materi)
                        <div class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <p class="text-xs text-gray-600">File saat ini:
                    ...
                    <p class="text-xs text-gray-600 mb-3">Biarkan kosong jika tidak ingin mengubah file</p>
