...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <tr class="hover:bg-gray-50">
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full"
                                             src="{{ $attempt->student->user->profile_photo_url ?? asset('images/default-avatar.png') }}"
                                             alt="{{ $attempt->student->user->name }}">
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                    ...
                                    {{ $attempt->status == 'submitted' ? 'Selesai' : 'Timeout' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="text-lg font-bold text-gray-900">{{ number_format($attempt->final_score, 1) }}</div>
                                <div class="text-sm text-gray-500">Skor: {{ $attempt->score }}/{{ $exam->questions->sum('score') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $grade = $attempt->final_score >= 85 ? 'A' :
                                            ($attempt->final_score >= 75 ? 'B' :
                                            ($attempt->final_score >= 65 ? 'C' :
                                            ($attempt->final_score >= 55 ? 'D' : 'E')));
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                <button onclick="resetAttempt({{ $attempt->id }})"
                                        class="text-red-600 hover:text-red-900">Reset</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
        ...
        <!-- Analisis Soal -->
        <div class="bg-white rounded-2xl shadow-lg p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Analisis Soal</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dijawab</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Benar</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Akurasi</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $question->answers_count }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $question->correct_answers_count }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex items-center">
                                    ...
                                    <span class="font-medium {{ $question->accuracy >= 70 ? 'text-green-600' : ($question->accuracy >= 40 ? 'text-yellow-600' : 'text-red-600') }}">
                                        {{ $question->accuracy }}%
                                    </span>
                                </div>
                            </td>
